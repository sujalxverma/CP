...
void solve() {
    ...
    for (ll j = 0; j < n; j++) cin >> w[j];

    ll sum = 0, left = 0, maxSum = 0;
    map<ll, ll> mp; // map from c-value to last index

    for (ll right = 0; right < n; right++) {
        if (mp.find(c[right]) != mp.end() && mp[c[right]] >= left) {
            // Remove elements from sum and update left pointer
            while (left <= mp[c[right]]) {
                sum -= w[left];
                left++;
            }
        }
