...
void solve()
{
    ...
    // Compute differences and sort in decreasing order
    vector<ll> diff(n);
    for (ll i = 0; i < n; i++)
        diff[i] = a[i] - b[i];
    sort(diff.rbegin(), diff.rend());

    // Merge both button types into single vector and sort
    vector<ll> buttons;
    buttons.insert(buttons.end(), white.begin(), white.end());
    buttons.insert(buttons.end(), black.begin(), black.end());
    sort(buttons.rbegin(), buttons.rend());

    // Greedily apply largest buttons to largest diffs
    int i = 0, j = 0;
    while (i < n && j < (m + k))
    {
        if (diff[i] < buttons[j])
