/*
* Euler tour technique, which transforms the tree into array, as per
* dfs traversal order of nodes.
* We can create subtree[], to store subtree for each node.
* And then use Segment tree to process subtree range query.

* Each node has an id(1,2,..n) and value associated to it.
*/

// CSES : Subtree Queries : https://cses.fi/problemset/task/1137/
// TC : O((N + Q) * log N)
// Update node X value to V, and find subtree for node X.
#include "bits/stdc++.h"
using namespace std;
