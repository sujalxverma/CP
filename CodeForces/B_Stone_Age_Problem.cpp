...
void solve() {
    ...
    vector<pair<ll, ll>> a(n);   // {value, timestamp}
    ll sum = 0;
    for (ll i = 0; i < n; i++) {
        ll val;
        cin >> val;
        a[i] = {val, 0}; // timestamp 0 for initial values
        sum += val;
    }

    pair<ll, ll> G = {-1, -1}; // global value + timestamp
    ll t = 1; // query counter (also used as timestamp)
    ...
    while (q--) {
        ...
        if (type == 1) {
            ...
            // Check: has this element been updated after last global set?
            if (a[idx].second >= G.second) {
                // old value from individual update
                sum += (x - a[idx].first);
            } else {
                // old value from last global set
                sum += (x - G.first);
            }
            // Set new value and update timestamp
            a[idx] = {x, t};
            ...
            ll x;
            cin >> x;
            G = {x, t}; // store new global value + timestamp
            sum = n * x;
            cout << sum << "\n";
        }
