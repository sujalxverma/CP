...
void dfs(vector<vector<ll>> &lake, vector<vector<ll>> &visited, pair<ll, ll> node, ll &volume)
{
    ...
    {
        ll newRow = row + dir.first;
        ll newCol = col + dir.second;

        if (newRow >= 0 && newRow < lake.size() && newCol >= 0 && newCol < lake[0].size())
        {
            if (visited[newRow][newCol] == -1 && lake[row][col] != 0)
            {
                dfs(lake, visited, {newRow, newCol}, volume);
            }
        }
    }
}
...
void solve()
{
    ll n, m;
    cin >> n >> m;
    vector<vector<ll>> lake(n, vector<ll>(m));
    rep2(i, 0, n)
    {
        rep2(j, 0, m)
        {
    ...
    rep2(i, 0, n)
    {
        ...
        {
            ll volume = 0;
            if (visited[i][j] == -1 && lake[i][j] != 0)
            { // not visited.

                dfs(lake, visited, {i, j}, volume);
