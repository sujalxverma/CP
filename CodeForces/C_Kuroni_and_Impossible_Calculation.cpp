...
//                                          Here you go
/*
    1. This problem is solved using pigeon hole principle.
    2. say if (n <= m) , then worst case, all elements are distcinct than we can simply O(n^2) and use of
        mod(m).
    3. but if say (n > m), then remainder lies b/w (0 to m-1), and then using pigeon hole principle, atleast two element will 
       exist whose remainder is same. and we take diff of them, then the remainder will be zero, then if multiply zero to the whole product, 
