...
void solve(){
    ...
    // no need for unordered_map; just check all-c quickly
    bool allc = true;
    for(int i = 0 ; i < n; i++){
        if(s[i] != ch){
            allc = false;
    ...
    // try to find a good x in [2..n]
    for(int i = 2; i <= n; i++){
        bool f = true;
        // check all multiples of i: k = i, 2i, 3i, ...
        for(int k = i; k <= n; k += i){
            // convert to 0-based index: s[k-1]
            if(s[k-1] != ch){
                f = false;
                break;
            }
    ...
    // no good x -> 2 ops with n and n-1
    cout << 2 << "\n";
    cout << n << " " << (n-1) << "\n";
    return;
}
