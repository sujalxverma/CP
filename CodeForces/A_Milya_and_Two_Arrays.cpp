...
ll countUnique(vl &a, ll n)
{
    std::unordered_set<ll> uniqueElements;
...
void solve()
{
    ...
    ll uniqueA = countUnique(a,n);
    ll uniqueB = countUnique(b,n);
    if(uniqueA >= 3 || uniqueB >= 3){
        yes;
        return ;
    }
    if(uniqueA >= 2 && uniqueB >=2){
        yes;
        return ;
    }
    no;
