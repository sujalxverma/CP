...
void f(string &s, int index, set<string> &ans) {
    if (index == s.length()) {
        ans.insert(s); // Insert the current permutation
        return;
    }

    for (int i = index; i < s.length(); i++) {
        swap(s[index], s[i]);           // Fix one character
        f(s, index + 1, ans);           // Recurse for the rest
