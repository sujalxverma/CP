...
//------------------------------------------------------------------------------------------------------------//
//                                          Here you go
// This solution leads to TLE ,need to work in future.
void solve() {
    int n, k;
    cin >> n >> k;
    vector<int> arr(n+1);
    ...
    for(int i = 1; i <= k; i++) colors[i].push_back(0);
    ...
    int ans = INT_MAX;
    for(int i = 1; i <= k; ++i) {
        vector<int>& x = colors[i];
        // If color doesn't occur in the array (just boundaries)
        ...
        int temp = pq.top(); // new maximum after split
        ans = min(ans, temp);
    }
    cout << ans << '\n';
}
