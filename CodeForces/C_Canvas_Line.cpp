...
void solve(){
    ...
    int m;
    cin>>m;
    vector<int>pagesLocation(m);
    for(int i = 0; i < m ; i++){
        cin >> pagesLocation[i];
    }

    // Now store pages location for each canvas.
    vector<vector<int>>location(n);
    vector<int>f(n);
    for(int i = 0; i<m;i++){
        for(int j = 0 ; j < n ;j++){
            if(pagesLocation[i]>=canvas[j].first && pagesLocation[i] <= canvas[j].second){
                f[j]++;
    ...
    for(int i = 0; i<m;i++){
        for(int j = 0 ; j < n ;j++){
            if(pagesLocation[i]>=canvas[j].first && pagesLocation[i] <= canvas[j].second){
               location[j].push_back(pagesLocation[i]);
            }
        }
    }
    // now we got location of each page in each canvas.
