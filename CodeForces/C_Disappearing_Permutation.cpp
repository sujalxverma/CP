...
void solve()
{
    ...
    // Step 1: Build DSU based on a[i] -> a[a[i]]
    for (int i = 1; i <= n; i++)
    {
        int u = a[i];
        int v = a[a[i]];
    ...
    // Step 2: Process in d[i] order and print cumulative unique component sizes
    set<int> seen_parents; // To avoid double counting
    ll cumulative_sum = 0;

    for (int i = 1; i <= n; i++)
    {
        int node = d[i];
        int par = dsu.superParent(node);

        if (seen_parents.count(par))
        {
            cout << cumulative_sum << " ";
        }
        else
        {
            cumulative_sum += dsu.size[par];
            cout << cumulative_sum << " ";
            seen_parents.insert(par);
        }
    }
