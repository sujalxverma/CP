...
void solve() {
    ...
    while (i >= 0) {
        if (a[i] == 'L' || a[i] == 'R') {
            ...
            // Count how many same characters in a
            while (i >= 0 && a[i] == ch) {
                count_a++;
                i--;
            }
            ...
            // Check if b has at least count_a and at most 2 * count_a
            if (!(count_b >= count_a && count_b <= 2 * count_a)) {
                cout << "No\n";
                return;
            }
        } else {
            // Unsupported character in a
            cout << "No\n";
            return;
        }
    }

    // After processing all of a, b must also be fully consumed
    if (j >= 0) {
        cout << "No\n";
    } else {
        cout << "Yes\n";
