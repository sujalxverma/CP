...
void solve()
{
    ...
    for (int i = 0; i < n - 1; i++)
    {
        ...
        else if (rem > y)
        {
            ...
            ll idx1 = lower_bound(b.begin() + i + 1, b.end(), mini) - b.begin();
            auto it = upper_bound(b.begin() + i + 1, b.end(), maxi);

            if (idx1 >= n) continue; // no elements ≥ mini
            ...
            // else: no element ≤ maxi → skip
        }
        else // x <= rem <= y
        {
             auto it = upper_bound(b.begin() + i + 1, b.end(), mini);
