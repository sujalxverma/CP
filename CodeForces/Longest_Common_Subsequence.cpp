...
int f(string a, string b, int idx1, int idx2,vector<vector<int>>&dp)
{
    ...
    if (a[idx1] == b[idx2])
    {
        return 1 + f(a, b, idx1 - 1, idx2 - 1,dp);
    }
    return dp[idx1][idx2]  = max(f(a, b, idx1 - 1, idx2,dp), f(a, b, idx1, idx2 - 1,dp));
}
