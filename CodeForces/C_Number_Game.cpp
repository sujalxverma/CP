...
int f(vector<int>& a, int k) {
    ...
    for (int i = 1; i <= k; i++) {
        ...
        if (it == s.begin()) {
            return 0; // Alice cannot find any value <= k - i + 1
        }
        --it;
        s.erase(it); // Alice removes one such value

        if (!s.empty()) {
            int min_value = *s.begin();
            s.erase(s.begin());
            s.insert(min_value + value); // Bob adds value to smallest element
        }
    }

    return 1; // Alice survived all k stages
}
