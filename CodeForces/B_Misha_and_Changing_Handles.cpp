...
void solve()
{
    int n;
    cin >> n;
    vector<pair<string,string>>usernames;
    map<string,int>mp;
    int index = 1;
    rep(i,0,n){
        // linking strings with number (as id).
        string a,b;
        cin >> a >> b;
        if(mp.find(a) == mp.end()){ // not present in map
            mp[a] = index;
            index++;
        }
        if(mp.find(b) == mp.end()){ // not present in map
            mp[b] = index;
            index++;
        }
        usernames.push_back({a,b});
    }
    map<int,int>names;
    DSU dsu(2*n);
    for(auto val : usernames){
        string oldName = val.first;
        string newName = val.second;

        int parent1 = dsu.superParent(mp[oldName]);
        int parent2 = dsu.superParent(mp[newName]);
       
        names[parent1] = mp[newName] ;
        dsu.unionBySize(parent1,parent2);

    }

    map<int,string>newMp;
    for(auto it : mp){
        newMp[it.second] = it.first;
    }

    cout<<names.size()<<endl;
    for(auto it : names){
        cout<<newMp[it.first]<<" "<<newMp[it.second]<<"\n";
    }
    return ;
