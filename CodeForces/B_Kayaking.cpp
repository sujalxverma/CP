...
int f(vector<int>&a , int rem , int selected, int index){
    
    if(a.size() <= index && rem != 0){
        return 1e8;

    }
    

    if(selected == 0){ // i can select;

        if(rem > 0){
            return min(-a[index] + f(a,rem-1,1,index+1) , -0 + f(a,rem,0,index+1),a[index] + f(a,rem+1,0,index+1));
        }
    }
    if(selected == 1){
        return min(a[index] + f(a,rem+1,0,index+1) , 0 + f(a,rem,1,index+1));
    }
